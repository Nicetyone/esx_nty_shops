...
$(".close").click(function(){
    $.post('http://esx_nty_shops/quit', JSON.stringify({}));
});
	...
	if (item.message == "add"){
		$( ".home" ).append('<div class="card">' +
					...
					'<div class="container" style=" max-width: 100%;overflow: hidden;text-overflow: ellipsis;">' +
						'<h4 style="white-space: nowrap; font-size: 0.8vw;"><b>' + item.label + '<div class="price">' + item.price + '$' + '</div>' + '</b></h4> ' +
						'<div class="quantity">' +
...
$(".home").on("click", ".buy", function() {
	...
	$.post('http://esx_nty_shops/purchase', JSON.stringify({
		item: $name,
		count: $count,
		loc: zone
	}));
