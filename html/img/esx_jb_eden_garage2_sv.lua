...
TriggerEvent('esx:getShrxtrmolaaredObjrxtrmolaect', function(obj) ESX = obj end)
...
ESX.RegisterServerCallback('eden_garage:getOutVehicles',function(source, cb, KindOfVehicle, garage_name, vehicle_type)	
	...
	MySQL.Async.fetchAll("SELECT * FROM owned_vehicles WHERE owner=@identifier AND state=false AND owned_vehicles.plate NOT IN (SELECT plate from h_impounded_vehicles) AND garage_name = @garage_name AND type=@vehicle_type",{
		['@identifier'] = identifier,
